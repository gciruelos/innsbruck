...
sortedLeadingMonomialLCM a b = map (\x -> if fst x > snd x then fst x else snd x) $
...
unrepeatedCartesianProduct :: Eq k => [Polynomial k] -> [(Polynomial k, Polynomial k)]
unrepeatedCartesianProduct gs = nubBy (\x y -> mapTuple terms x == mapTuple terms y ||
                                 mapTuple terms (swap x) == mapTuple terms y) 
                                [(p, q) | p<-gs, q<-gs, terms p /= terms q]

groebner' :: (Eq k, Fractional k) => MonomialOrdering -> [Polynomial k] -> [Polynomial k]
...
groebner :: (Eq k, Fractional k) => MonomialOrdering -> [Polynomial k] -> [Polynomial k] 
groebner order fs = until (\gs -> map terms (groebnerStep order gs) == map terms gs) (groebnerStep order) fs
